...
let SummaryNgTable = React.createClass({
	goBack(){
		history.back();
	},
	componentDidMount() {
		...
		if(query.time){
			this.props.dispatch(actions.summaryTableNgActionAsync({
				jgzj:'全部',
...
SummaryNgTable = connect(mapStateToProps)(SummaryNgTable);

export default SummaryNgTable;