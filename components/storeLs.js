...
let StoreLs = React.createClass({
	handleSearch(){
		let gdzzh = document.getElementById('gdzzh').value;
		let jgzj = document.getElementById('zj').value;
		let storeTableData = {
	...
	componentDidMount() {
		let selectOptions = {
			...
			jgzj:document.getElementById('zj').value,
			type:'2'
		}
							...
							>
								{
									this.props.storeTable.columns.map(function(item){
										if(item.dataIndex == 'gdzzh' || item.dataIndex == 'zj' || item.dataIndex == 'lb' || item.dataIndex == 'lp' || item.dataIndex == 'gh'){	
											return  <Column
														title={item.title}
...
StoreLs = connect(mapStateToProps)(StoreLs);
